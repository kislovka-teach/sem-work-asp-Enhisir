...
function Company({
  companyInfo,
  enableUpperBorder = false,
}: {
  companyInfo: WorkPlace;
  enableUpperBorder?: boolean;
}) {
  const monthNames = [
    "Январь",
    "Февраль",
  ...
  return (
    <div
      className={[
        classes.companyBlock,
        enableUpperBorder ? classes.borderTop : "",
      ]
        .join(" ")
        .trim()}
    >
      <div className={classes.dates}>
        <p>
          {monthNames[dateBegin.getMonth() - 1]} {dateBegin.getFullYear()} -
        </p>
        <p>
          {dateEnd
