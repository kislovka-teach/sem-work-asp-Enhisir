...
function ResumeFilter() {
  ...
  const [leftSalaryBorder, setLeftSalaryBorder] = useState<
    number | undefined
  >();
  const [rightSalaryBorder, setRightSalaryBorder] = useState<
    number | undefined
  >();
  ...
  useEffect(() => {
    api
      .get("/resumes/tags" + (searchString ? `?name=${searchString}` : ""))
      .then((response) => {
        setAvaliableOptions(
  ...
  useEffect(() => {
    ...
    if (leftSalaryBorder)
      newSearchParams["leftSalaryBorder"] = leftSalaryBorder;
    if (rightSalaryBorder)
      newSearchParams["rightSalaryBorder"] = rightSalaryBorder;
    if (currentGradeOptions.length > 0)
      newSearchParams["grade"] = currentGradeOptions[0].value;
    if (addedOptions.length > 0)
      newSearchParams["tags"] = addedOptions.flatMap(
        (item: OptionContainer) => item.value,
      );
